...
db.Talent = thinky.createModel('eote_talents', {
    ...
    name: type.string().required(),
    description: type.string().required(),
    activation: type.string().enum(['p', 'ai', 'aio', 'am', 'aa']).default('p'),
...
db.Character = thinky.createModel('eote_characters', {
    ...
    age: type.number().integer().min(1).required(),
    height: type.string(),
    gender: type.string().enum(['male', 'female', 'other', 'n/a']).default('other'),
    species: type.string().required(),
    characteristics: type.object().schema({
        brawn: type.number().integer().min(1).default(2),
        agility: type.number().integer().min(1).default(2),
        intellect: type.number().integer().min(1).default(2),
        cunning: type.number().integer().min(1).default(2),
        willpower: type.number().integer().min(1).default(2),
        presence: type.number().integer().min(1).default(2)
    }),
    experience: type.object().schema({
        total: type.number().integer().min(0).default(0),
        available: type.number().integer().min(0).default(0)
    }),
    defenses: type.object().schema({
        soak: type.number().integer().min(0).default(0),
        melee: type.number().integer().min(0).default(0),
        ranged: type.number().integer().min(0).default(0)
    }),
    force: type.object().schema({
        rank: type.number().integer().min(0).default(0),
        pool: type.number().integer().min(0).default(0),
        committed: type.number().integer().min(0).default(0),
        powers: type.array().schema({
            index: type.number().integer().min(0).required(),
            rank: type.number().integer().min(0).default(0)
        })
    }),
    health: type.object().schema({
        wounds: type.number().integer().min(0).default(0),
        woundThreshold: type.number().integer().min(0).default(0),
        strain: type.number().integer().min(0).default(0),
        strainThreshold: type.number().integer().min(0).default(0),
        criticals: type.array().schema(type.string())
    }),
    skills: type.object().schema({
        astrogation: type.object().schema({
            ranks: type.number().integer().min(0).default(0),
    ...
    extraSkills: type.array().schema({
        name: type.string().required(),
        type: type.string().enum(['general','knowledge','combat']).default('general').required(),
        characteristic: type.string().enum(['brawn', 'agility', 'intellect', 'cunning', 'willpower', 'presence']).required(),
        ranks: type.number().integer().min(0).default(0),
        career: type.boolean().default(false)
    }),
    ...
    armor: type.object().schema({
        name: type.string().required(),
        defense: type.number().integer().min(0).default(0),
        soak: type.number().integer().min(0).default(0),
        hardpoints: type.number().integer().min(0).default(0),
        encumbrance: type.number().integer().min(0).default(0)
    }),
    weapons: type.array().schema({
        name: type.string().required(),
        skill: type.string().required(),
        damage: type.number().integer().min(0).default(0),
        critical: type.number().integer().min(0).default(0),
        range: type.string().enum(["engaged", "short", "medium", "long", "extreme"]).default('general').required(),
